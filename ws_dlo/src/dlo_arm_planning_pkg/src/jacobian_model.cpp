...
namespace dlo_arm_planning_pkg
{
    ...
    {
        if (!b_load_weights_)
            ROS_WARN("JacobianModel: haven't loaded network weights.");
        ...
        const double &dlo_length)
    {
        ...
        // discard the data with too large velocity
        ...
        {
            online_dataset_.erase(online_dataset_.begin()); // remove the first element
        }
        ...
        for (size_t iter = 0; iter < (online_update_rate_ / online_data_rate_); iter++)
        { // update rate >> data rate, to make the updating converge better when using large learning rate
            // compute the prediction error and other terms
            ...
            // update the weights (according to the equations in the TRO paper)
            for (size_t k = 0; k < num_fps_; k++)
            {
