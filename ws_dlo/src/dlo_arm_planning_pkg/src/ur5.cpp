...
// ------------------------------------------------------------
void UR5::forward(const double* q, double* T) {
...
// ------------------------------------------------------------
void UR5::forward_all(const double* q, double* T1, double* T2, double* T3, 
...
// ------------------------------------------------------------
int UR5::inverse(const double* T, double* q_sols, double q6_des) {
    int num_sols = 0;
...
// ------------------------------------------------------------
int UR5::inverse(
  const Eigen::Isometry3d &tool0_pose, // pose of tool0
  std::vector<std::vector<double> > &q_solutions, // ~[0~2pi]
  double q6_des
){
  // analytical solver 定义的 end effector frame 不是 tool0，但没找到能和哪个frame对应上。这里进行手动坐标变换。
  Eigen::Isometry3d ee_pose_in_tool = Eigen::Isometry3d::Identity();
  ee_pose_in_tool.rotate(Eigen::Quaterniond(Eigen::Vector4d(0.5, -0.5, 0.5, 0.5)));
  Eigen::Isometry3d ee_pose = tool0_pose * ee_pose_in_tool;
  ...
  double q_sols[8*6];
  int n_sols = inverse(T, q_sols, q6_des);

  q_solutions = std::vector<std::vector<double> >(n_sols);
  for(int i = 0; i < n_sols; i++){
    for(int j = 0; j < 6; j++){
      q_solutions[i].push_back(q_sols[i * 6 + j]);
    }
  }
