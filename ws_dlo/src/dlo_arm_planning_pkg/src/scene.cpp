...
namespace dlo_arm_planning_pkg
{
    ...
    Scene::Scene(
        ...
        const planning_scene::PlanningSceneConstPtr &planning_scene) : nh_(nh)
    {
        dlo_ = dlo;
        dual_arm_ = dual_arm;
        cd_fcl_ = std::make_shared<CollisionDetectionFCL>(nh_, dlo_, dual_arm_);
        ...
        const Eigen::VectorXd &arm_1_joint_pos)
    {
        ...
        current_state.update(); // update transformations for collision check
    ...
    bool Scene::checkDLOAndWorldCollision(
        ...
        double min_dist_thres)
    {
        if (dlo_state.fps_pos_.size() == 0)
            ROS_ERROR("Scene::checkDLOAndWorldCollision(): the input dlo_state.fps_pos_ is empty.");
        ...
        const Eigen::Vector3d &point)
    {
        ...
        // indices of the adjacent 8 vertices
        ...
        // compute the coordinate of each vertex and the distance to obstacle
    ...
    {
        if (fps_pos.size() == 0)
            ROS_ERROR_STREAM("Scene::minDistBetweenDLOAndWorld(): empty input fps_pos");
