...
namespace dlo_arm_planning_pkg
{
    ...
    {
        if (b_received_dlo_rt_obs_ == false)
        {
            // set the DLO length when receiving the DLO observation message at the first time
    ...
    bool RealTimeInterface::moveAndGraspDLO()
    {
        ...
        // use IK to compute the robot joint position to grasp the DLO ends
        ...
        // use MoveGroup to plan the robot trajectories
        ...
        // establish the 'attachment' between DLO end and robot end effector in Unity
